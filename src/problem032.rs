...
fn get_pandigital_products() -> i32 {
    ...
    for f1 in 1..1000 {
        for f2 in (f1 + 1)..10000 {
            ...
            let mut vec_number = format!("{f1}{f2}{product}").chars().collect::<Vec<char>>();

            if vec_number.len() < 9 {
                continue;
            } else if vec_number.len() > 9 {
                break;
            }

            vec_number.sort();
            let str_number = vec_number.into_iter().collect::<String>();
            if str_number == "123456789" {
